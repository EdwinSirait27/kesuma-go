@extends('index')
@section('title', 'Kesuma-GO | Data Status Pegawai')
@section('content')
    @include('statuspegawai.create')
    @include('statuspegawai.jsdnjds')
    ...
    <div class="row" id="hal_index">
        <div class="col-md-12 col-sm-12">
            ...
            <div class="x_panel">
                <div class="x_title">
                    <h2><i class="fa fa-book" style="margin-right: 10px; margin-top: 15px;"></i>Data Status <small>Pegawai</small></h2>
    ...
    <script>
        ...
        function editAndShow(iv, id) {
            $.ajax({
                url: "/statuspegawai-edit/" + id,
                type: "GET",
                success: function(data) {
                    // Mengisi nilai input dengan data yang ada
                    $('#txt_id').val(id);
                    $('#txt_statuspegawai').val(data.txt_statuspegawai);
        ...
        function saveChanges() {
            var id = $('#txt_id').val();
            var statuspegawai = $('#txt_statuspegawai').val();
        ...
        $(document).on('click', '#bulk_delete', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (id.length > 0) {
                        $.ajax({
                            url: "{{ route('statuspegawai.removeall') }}", // Hapus 'kurikulum_id' => ''
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            method: "get",
                            data: {
                                status_id: id // Ganti 'kurikulum_id' dengan 'id_kur'
                            },
                            success: function(data) {
                                console.log(data);
                                Swal.fire({
                                ...
                                window.location.assign("statuspegawai");
                            },
                            error: function(data) {
