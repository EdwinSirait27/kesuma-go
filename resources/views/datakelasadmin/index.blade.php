...
<div class="row">
    ...
    $.ajax({
        ...
        function showSiswaInTable(kelas_id) {
            $.ajax({
                url: "/datakelasadmin-edit/" + kelas_id,
                type: "GET",
        ...
        function showSiswaInTable1(kelas_id) {
            $.ajax({
                url: "/datakelasadmin-edit1/" + kelas_id,
                type: "GET",
                success: function(siswas) {
                    $('#siswaTerdaftarTable tbody').empty();
                    $.each(siswas, function(index, siswa) {
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var table = $('.user_datatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('datakelasadmin.index') }}",
                    method: "GET"
                },
                columns: [{
                        data: 'datakelas_id',
    ...
    render: function (data, type, row) {
        return data; // Untuk menampilkan data yang sudah kita siapkan di controller
                    ...
                    {
                        data: 'tahun.statusaktif',
                        name: 'tahun.statusaktif'
                    },
            ...
            $('#tahun_akademik_filter').on('change', function() {
                ...
                table.ajax.url("{{ route('datakelasadmin.index') }}?tahunakademik_id=" + tahunAkademikId).load(); // Mengubah URL Ajax dan memuat ulang tabel
            });
        });
        $(document).on('click', '#bulk_delete', function() {
            ...
            Swal.fire({
                title: "Apakah Yakin?",
                text: "Pastikan untuk menghapus data jadwal yang ada pada menu jadwal terlebih dahulu",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    ...
                    if (id.length > 0) {
                        $.ajax({
                            url: "{{ route('datakelasadmin.removeall') }}", // Hapus 'kurikulum_id' => ''
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            method: "get",
                            data: {
                                datakelas_id: id // Ganti 'kurikulum_id' dengan 'id_kur'
                            },
                            success: function(data) {
                                console.log(data);
                                Swal.fire({
