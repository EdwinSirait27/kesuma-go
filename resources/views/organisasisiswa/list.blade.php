@extends('index')
@section('title', 'Kesuma-GO | Data Oganisasi')
@section('content')
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card shadow">
                <div class="card-header bg-dark text-white">
                    <h3 class="text-center mb-0">List Organisasi</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('organisasisiswa.storee') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            <label class="font-weight-bold">Pilih Organisasi:</label><br>
                            @foreach ($organisasigurus as $organisasiguru)
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" name="organisasi_guru_siswa_id[]" value="{{ $organisasiguru->organisasi_guru_siswa_id }}" id="organisasi{{ $organisasiguru->organisasi_guru_siswa_id }}">
                                    <label class="form-check-label" for="organisasi{{ $organisasiguru->organisasi_guru_siswa_id }}">{{ $organisasiguru->organ->nama }}</label>
                                </div>
                            @endforeach
                        </div>
